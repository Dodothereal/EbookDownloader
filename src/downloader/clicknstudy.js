...
function clicknstudy(email, passwd, deleteAllOldTempImages) {
    ...
    if(email === "token") {
        cookieJar.setCookie(tough.Cookie.parse("CASSID=" + passwd + '; path=/; domain=click-and-study.de'), 'https://www.click-and-study.de').then(() => {
            axiosInstance({
                method: "get",
                url: "https://www.click-and-study.de/Buecher",
            }).then(handleBuecher);
        })
        return;
    }
